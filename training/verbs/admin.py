...
class VerbAdmin(admin.ModelAdmin):
    ...
    inlines = [InfoInline, ExampleInline]
    search_fields = [
        "infinitive",
        "simple_past",
        "past_participle",
    ...
    search_help_text = (
        "Search verbs by infinitive, simple past, past participle or translation"
    )
    ...
    def get_queryset(self, request: HttpRequest) -> models.QuerySet[Verb]:
        queryset = super().get_queryset(request)
        return queryset.select_related("similarity")
    ...
    def get_search_results(
        ...
        queryset = (
            ...
            .verbs.exclude(id__in=excluded_verbs)
            .filter(
...
@admin.register(Info)
class InfoAdmin(GetReadOnlyFieldsMixin, admin.ModelAdmin):
    list_display = [
        "get_verb",
        "content",
    ]
    ...
    search_help_text = "Search info by verb infinitive."
    ...
    @admin.display(ordering="verb__infinitive", description="Verb")
...
class ExampleAdmin(GetReadOnlyFieldsMixin, admin.ModelAdmin):
    list_display = [
        "get_verb",
        "english",
        "translation",
    ...
    readonly_fields = ["verb"]
    search_fields = ["verb__infinitive"]
    search_help_text = "Search examples by verb infinitive."
    autocomplete_fields = ["verb"]
    ordering = ("verb__infinitive",)
    list_per_page = 50
...
class SimilarityAdmin(admin.ModelAdmin):
    list_display = ["name"]
    search_fields = ["name"]
    search_help_text = "Search similarities by name."
    list_per_page = 50
